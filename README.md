...
**Standalone HOME Menu packaging is now available.** A locally built CIA installs through FBI and adds a Melee disc icon, an animated 3D Final Destination banner with two Foxes, and the intro announcer call. It reuses the existing SD game files. The first two CIA banners caused a crash/freeze during physical HOME unwrapping. HOME package 3 substantially reduces the model/animation workload while preserving the scene, adds stricter binary checks, and retains the full-color disc icon. Physical HOME validation is still pending. [Installation and local banner authoring](native-3ds/docs/HOME_MENU.md).
