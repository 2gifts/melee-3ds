...
**Standalone HOME Menu packaging is now available.** A locally built CIA installs through FBI and adds a Melee disc icon, an animated 3D Final Destination banner with two Foxes, and the intro announcer call. It reuses the existing SD game files. HOME package 2 corrects the first CIA’s unwrapping crash with rigid banner animation, fixed bindings and extended-banner metadata, and uses full-color disc artwork. Physical HOME Menu validation of this correction is pending. [Installation and local banner authoring](native-3ds/docs/HOME_MENU.md).
