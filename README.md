...
**Standalone HOME Menu packaging is experimental.** HOME package 4 was confirmed to unwrap and display on a physical New 3DS. Package 5 retains its compact rigid diorama, restores the missing launch splash, corrects the colors and title framing, grounds an idle Fox and a taunting Fox on Final Destination, and mixes the announcer over the menu theme. Local renderer and installed-executable checks pass; physical launch confirmation is still pending. [Installation, authoring and test limits](native-3ds/docs/HOME_MENU.md).
