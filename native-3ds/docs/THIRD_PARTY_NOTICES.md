...
Optional simplified stage assets are from [Diet Melee Classic](https://diet.melee.tv/), patched locally from the user's ISO. Original settings and selected animation data are restored for native compatibility; these modifications are documented in `OFFLINE_PERFORMANCE_UPDATE_4.md` and the generated archive audits. The official 1.0.3 patch's stage archives were verified identical to the 1.0.2 inputs. The user's original disc assets are preserved separately. No game data has been published or uploaded.
