...
the game executable, a Melee mini-disc icon, an animated 3D Final Destination
...
authoring and packaging source; disc-derived banner images, meshes, audio,
...
4. Return to HOME, unwrap the new gift if shown, and launch the Melee disc icon.
...
python tools/make_home_menu_art.py
...
banner animation reuses the captured poses, with the fighters enlarged and
...
- Banner: 1,702 triangles, a looping 3.2-second skeletal animation, and a
  520,376-byte CGFX under HOME's 524,288-byte limit. Mesh deduplication and
