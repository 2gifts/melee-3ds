...
The batched triangle command packet in `citro3d_fix.c` adapts
`source/drawElements.c` at the same commit, under `CITRO3D-LICENSE.txt`.
Development builds can compare each packet byte for byte with commands
generated by the linked SDK before submitting it. Texture binding reuse
preserves the SDK's dirty-state path and explicitly invalidates on uploads.
