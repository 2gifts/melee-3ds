/* Compare identical current shaders with the unlit shortcut off/on.
 * Lit, general, mixed-material and CPU clip-space-marker cases interleave. */
static volatile unsigned unlit_affine_verify,unlit_affine_verify_page;
static void verify_unlit_affine_paths(void){
    if(!stereo_active)return;
    unsigned token=unlit_affine_verify,lighting=lighting_uniform_disable,disabled=unlit_affine_disable;
    unsigned shortcuts=shader_shortcuts_disable;shader_shortcuts_disable=0;unlit_affine_disable=0;
    ...
    target=test;
    for(unsigned page=0;page<16;++page){unlit_affine_verify_page=page;
        for(unsigned mode=0;mode<2;++mode){
            lighting_uniform_disable=0;unlit_affine_disable=mode==0;
            unsigned before[2];memcpy(before,unlit_affine_draws,sizeof(before));
            ...
            for(unsigned cell=0;cell<48;++cell){
                ...
                for(unsigned i=0;i<4;++i){
                    ...
                    v[i].n[0]=.15f;v[i].n[1]=.1f;v[i].n[2]=1;v[i].n[3]=(item%11==0&&i%2)?-1:row;
            ...
            for(unsigned i=0;i<2;++i)routes[mode][i]+=unlit_affine_draws[i]-before[i];
            ...
            char path[100];snprintf(path,sizeof(path),"sdmc:/3ds/melee/unlit-affine-fixture-%u-%u.bin",page,mode);
    ...
    FILE*f=fopen("sdmc:/3ds/melee/unlit-affine-fixture-report.json","wb");if(!f)mp_native_panic("Affine report failed");
    ...
    shader_shortcuts_disable=shortcuts;lighting_uniform_disable=lighting;unlit_affine_disable=disabled;
    target=saved;C3D_FrameDrawOn(target);screen_viewport(320);raster_state_invalidate();unlit_affine_verify=0;
}
