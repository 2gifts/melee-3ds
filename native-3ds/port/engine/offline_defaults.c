...
void mp_offline_defaults(void){
    ...
    r->friendly_fire=1;r->pause=0;r->score_display=0;
    p->item_freq=255;p->item_mask=0;
    /* Slippi's SinglesStages selects the six legal stages for Random. */
    p->stage_mask=0xe70000b0u;
    OSReport("Offline defaults: all characters/stages, 4 stocks, 8 minutes, items off, team attack on, pause off; UCF 0.84 enabled\n");
}
