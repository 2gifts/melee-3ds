...
def main():
    ...
    for name,data in zip(('expected','actual'),images):Image.fromarray(data).save(ROOT/f'build/points-{name}.png')
    ...
    (ROOT/'build/point-gpu-test.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
