"""Compare all RGBA channels for identity and general affine materials."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',default='build/affine-identity-qa/gpu')
    ...
    if not args.resume:
        set_word('mp_test_frame_limit',0);set_word('mp_test_stereo_slider',1000);set_word('affine_verify',token)
    deadline=time.monotonic()+180
    while True:
        time.sleep(.5);state=read(('engine_failed','affine_verify','affine_verify_page'))
        assert not state['engine_failed'],state
        if not state['affine_verify']:break
        if time.monotonic()>deadline:raise TimeoutError(('Fixture remains live; resume this token',token,state))
    report=json.loads((SD/'affine-fixture-report.json').read_text())
    assert report==dict(token=token,pages=16,cases=768,routes=[[768,0],[192,576]]),report
    pages=[];maximum=changed=0
    for page in range(16):
        arrays=[np.fromfile(SD/f'affine-fixture-{page}-{mode}.bin',dtype=np.uint8).reshape(800,240,4) for mode in range(2)]
    ...
    result=dict(passed=maximum==0,pages=pages,compared_channels=16*800*240*4,changed_channels=changed,
        maximum_error=maximum,fixture=report,physical_fps_verified=False)
